...
function Login() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const user = await dispatch(loginThunk({email: form.email, password: form.password })).unwrap();
        } catch (err) {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
            <form onSubmit={handleSubmit} className="w-full max-w-md p-8 rounded-2xl shadow-2xl bg-gray-800 border border-gray-700 flex flex-col gap-4">
                <h2 className="text-3xl font-bold text-center text-cyan-400 mb-4">Login</h2>
